...
<html lang="en">
 ...
 <div class="container-fluid">
		...
		<?php
		if(!empty($p)): include "../../frontend/view_download.php";
		...
		?>
		  <div class="span12" id="containt">
			...
			<div class="row-fluid" id="content">
				<ul class="postings">
					<?php
					$sqlnews="select id_download,judul from download where status='Publish'";
					if(!empty($key)): $sqlnews.=" and judul like '%$key%' and status='Publish' and isi like '%$key%' order by id_download desc";
					else: $sqlnews.=" order by id_download desc";
					endif;
									
					$qnews=mysqli_query($db,$sqlnews);
					echo mysqli_error($db);
					while(list($idn,$jdln)=mysqli_fetch_row($qnews)):
					?>
				<li class="span6 posting" style="padding: 5px; min-height: 165px; max-height: 160px; display: block; overflow: auto;">
						
						<h4><a href="download.php?p=<?php echo $idn; ?>"><?php echo $jdln; ?></a></h4>
						
						<p><a href="download.php?p=<?php echo $idn; ?>" class="btn btn-inverse">readmore &raquo;</a></p>
